...
<xsl:template match="/LineSET_LIST">
  ...
  <xsl:if test="not($status1='')">
	 <xsl:apply-templates select="LS[SUBMIT_STATUS=$status1]"/>
  </xsl:if>   
  <xsl:if test="not($status2='')">
	 <xsl:apply-templates select="LS[SUBMIT_STATUS=$status2]"/>
  </xsl:if>   
  <xsl:if test="not($status3='')">
	 <xsl:apply-templates select="LS[SUBMIT_STATUS=$status3]"/>
  </xsl:if>   
  <xsl:if test="not($status4='')">
	 <xsl:apply-templates select="LS[SUBMIT_STATUS=$status4]"/>
  </xsl:if>   
  <xsl:if test="not($status5='')">
	 <xsl:apply-templates select="LS[SUBMIT_STATUS=$status5]"/>
  </xsl:if>   		 	
  </xsl:copy>	
</xsl:template>
