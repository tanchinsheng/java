...
<form name="showPacketDetails" action="<%=contextPath%>/packetServlet" method="post">
    <input type="hidden" name="<%=AfsServletUtils.ACTION_FIELD_NAME%>" value="<%=PacketServlet.VIEW_RECIEVED_PACKET_ACTION%>">
...
<% boolean repBool = false; boolean csvRepBool = false; String pageTitle = "Received Sent Packages"; %>
...
<!--intestazione -->
    <TR bgColor="#c9e1fa">
      ...
      <TD class="testoH"><B><%=PacketConstants.FIRST_RECIEVE_LOGIN_LABEL%></B></TD>
      <TD class="testoH"><B><%=PacketConstants.SECOND_RECIEVE_LOGIN_LABEL%></B></TD>
...
%>
    <!--riga -->
            <TR>
              ...
              <TD class="testoD"><%=currentPacket.getFromPlant()%></TD>
              <TD class="testoD"><%=currentPacket.getFormattedSentDate()%></TD>
              <TD class="testoD"><%=currentPacket.getFirstRecieveLogin()%></TD>
              <TD class="testoD"><%=currentPacket.getSecondRecieveLogin()%></TD>
    ...
    <%}%>
            ...
            <tr>
                <td colspan="8"><img src="<%=contextPath%>/img/blank_flat.gif" alt="" border="0"></td>
            </tr>
            <tr>
