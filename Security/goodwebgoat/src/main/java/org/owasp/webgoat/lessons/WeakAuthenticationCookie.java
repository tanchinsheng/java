...
public class WeakAuthenticationCookie extends LessonAdapter
{
	...
	protected String checkCookie(WebSession s) throws Exception
	{
		...
		if (cookie != null)
		{
			if (cookie.equals(encode("webgoat12345")) && 
                                (s.getParser().getStringParameter(USERNAME, "").equals("webgoat"))) 
                            { return ("webgoat"); }
			if (cookie.equals(encode("aspect12345")) && 
                                (s.getParser().getStringParameter(USERNAME, "").equals("aspect"))) 
                            { return ("aspect"); }
                        /* Assume hacker doesn't know alice login credential.
                         * On checking the encoded cookie, we know the user should be alice.
                         * Check that alice is the person login.
                         */
			if (cookie.equals(encode("alice12345")) &&
                                (s.getParser().getStringParameter(USERNAME, "").equals("alice"))) 
			{
				makeSuccess(s);
				return ("alice");
			}
	...
	protected String checkParams(WebSession s) throws Exception
	{
		...
		if ((username.length() > 0) && (password.length() > 0))
		{
			...
			if (loginID != "")
			{
				...
				s.setMessage(WebGoatI18N.get("IdentityRemembered") + " " + loginID);
	...
	protected Element makeLogin(WebSession s)
	{
		...
		t.addElement(new TR(new TD(b)));
                t.addElement(new TR(new TD((new IMG("images/ST/secure.jpg").setWidth(38).setHeight(20)))));
		ec.addElement(t);
