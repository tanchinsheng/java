...
class Person extends Thread {
    ...
    public void run() {
        try {
            System.out.println("[" + machines.availablePermits() + "]" + getName() + " waiting to access an ATM machine");
            machines.acquire();
            System.out.println("[" + machines.availablePermits() + "]" + getName() + " is accessing an ATM machine");
            Thread.sleep(1000); // simulate the time required for withdrawing amount
            System.out.println("[" + machines.availablePermits() + "]" + getName() + " is done using the ATM machine");
            machines.release();
