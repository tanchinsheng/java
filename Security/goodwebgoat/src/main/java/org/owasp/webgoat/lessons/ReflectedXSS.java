...
public class ReflectedXSS extends LessonAdapter {
    ...
    protected Element createContent(WebSession s) {
        ...
        try {
            ...
            TR rowImage = new TR();
            rowImage.addElement(new TD().addElement(new IMG("images/ST/secure.jpg").setWidth(38).setHeight(20)).setColSpan(2));
            t.addElement(rowImage);

            tr = new TR();
            tr.addElement(new TD().addElement("Enter &lt;script&gt;alert('Bang!')&lt;/script&gt; for the PIN value").setColSpan(2));
            t.addElement(tr);
            ...
            s.setMessage(WebGoatI18N.get("ErrorGenerating") + this.getClass().getName()
                    + ". Detected XSS! or Invalid pin (must be 3 numeric)!");
            e.printStackTrace();
            /* End Good Code */
        } catch (Exception e) {
            s.setMessage(WebGoatI18N.get("ErrorGenerating") + this.getClass().getName());
     ...
     */
    /* Start Good Code */
    protected String validate(final String parameter, final Pattern pattern, WebSession s) throws ValidationException {
