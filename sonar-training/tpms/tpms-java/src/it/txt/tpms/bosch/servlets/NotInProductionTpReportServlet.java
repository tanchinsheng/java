 ...
 * Time: 9.13.14
 */
public class NotInProductionTpReportServlet extends AfsGeneralServlet {
        ...
        if (!GeneralStringUtils.isEmptyString(action)) {
            if (action.equals(PERFORM_SEARCH_ACTION)) {
                ...
                try {
                    ...
                    errorLog("InProductionTpReportServlet :: doPost : unable to parse LAST_ACTION_FROM_FIELD_NAME, parameter will not be considered");
                }
                ...
                try {
                    ...
                    errorLog("InProductionTpReportServlet :: doPost : unable to parse LAST_ACTION_TO_FIELD_NAME, parameter will not be considered");
                }
                ...
                if ( !GeneralStringUtils.isEmptyString( selectedFilterId ) ) {
                    ...
                    if ( filterPos >= 0 ) {
                        debugLog( "InProductionTpReportServlet :: doPost : filterDisplayValue founded = " + ( ( String[] )userLsFilters.get( filterPos ) )[1] );
                        filterDisplayValue = ( ( String[] )userLsFilters.get( filterPos ) )[1];
                    } else {
                        debugLog( "InProductionTpReportServlet :: doPost : filterDisplayValue NOT founded " );
                    }
                }
                TPList tpList;
                try {
                    tpList = BoschReportsManager.getNotInProductionTPs(fromPlant, toPlant, division, productLine, jobName, lastActionDateFrom, lastActionDateTo, filterDisplayValue, currentUser);
                } catch (TpmsException e) {
                    manageError(e, request, response);
                    return;
                }
                debugLog( "InProductionTpReportServlet :: doPost : " + (tpList != null ? tpList.size() : -3));
...
debugLog( "InProductionTpReportServlet :: initializeFormattedReportSearchCriteria : formattedSeachCriteria = " + formattedSeachCriteria );
    }

    public static boolean showExportIcons(HttpSession session) {
        TPList inProductionTpList = (TPList) session.getAttribute(TP_LIST_SESSION_ATTRIBUTE_NAME);
        return (inProductionTpList != null && !inProductionTpList.isEmpty());
    }

    private void initializeSearchListValues(HttpSession session) {
        String commonErrorMessage = "InProductionTpReportServlet :: initializeSearchListValues";
